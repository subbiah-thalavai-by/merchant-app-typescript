...
const defaultTaxes: IProductType[] = [];
...
const ProductTypeComp: React.FC = () => {
  const history = useHistory();
  const classes = useStyles();
  const [productTypes, setProductTypes]: [IProductType[], (Taxes: IProductType[]) => void] = React.useState(defaultTaxes);
  // const [loading, setLoading]: [boolean, (loading: boolean) => void] = React.useState<boolean>(true);
  // const [error, setError]: [string, (error: string) => void] = React.useState('');
  const [page, setPage] = React.useState(0);
  const [rowsPerPage, setRowsPerPage] = React.useState(10);

  const emptyRows = rowsPerPage - Math.min(rowsPerPage, productTypes.length - page * rowsPerPage);
    ...
    <>
      ...
      <TableContainer component={Paper}>
        <Table aria-label="custom pagination table">
          ...
          <TableBody>
            {(rowsPerPage > 0
              ? productTypes.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
              : productTypes
            ).map((row) => (
