...
const CreateProductTypeComp: React.FC = () => {
  ...
  const saveProductType = async () => {
    ...
    if (!error) {
      setProgress(0);
      const productTypeData = await AddProductTypeData();
      setProgress(100);
  ...
  const AddProductTypeData = async () => {
    let returnValue;
    const productTypeObj = {
      title: productType.title,
    };
    try {
      const response = await axios.post(`${process.env.REACT_APP_BASE_URL}product-types`, productTypeObj);
      returnValue = 'success';
    } catch (e: any) {
      setErrorOpen(true);
          ...
          >
            <Grid item xs={12} spacing={1}>
              <Typography component="div" className={classes.pageTitle}>
                {propertiesfile.title_product_type_create}
              </Typography>
            </Grid>
...
export default CreateProductTypeComp;
